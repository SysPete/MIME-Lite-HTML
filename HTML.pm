...
sub parse
     {     
     my($self,$url_page,$url_txt)=@_;
     my ($type,@mail,$gabarit,$gabarit_txt);
     ...
     if ($url_txt)
          ...
          if (!$res3->is_success) {die "$url_txt n'est pas accessible";}
     ...
     foreach my $url (@l) 
          ...
          if (!$res2->is_success) {print "Can't get $urlAbs\n";}   
          # Create part
          my $mail = new MIME::Lite 
               Data => $res2->content,
               Encoding =>'base64';          
